define("./list.tpl.html", [ "require", "exports", "module" ], function(require, exports, module) {
    ...
    exports.render = function($data, $opt) {
        ...
        (function() {
            with ($data) {
                _$out_.push('<style type="text/css">.clearfix { *zoom: 1;}.clearfix:before,.clearfix:after { display: table; content: ""; line-height: 0;}.clearfix:after { clear: both;}label.auto-complete-mockup-checkbox-label { line-height: 30px; min-width: 80px; max-width: 260px; vertical-align: top; font-weight: normal; color: #333; margin: 0; cursor: pointer; position: relative; line-height: 24px; padding-left: 28px;}label.auto-complete-mockup-checkbox-label label.auto-complete-mockup-checkbox { position: absolute; margin: 0; left: 0; top: 0;}label.auto-complete-mockup-checkbox { width: 24px; margin: 0; overflow: hidden; cursor: pointer;}label.auto-complete-mockup-checkbox input { position: absolute; left: -999px;}label.auto-complete-mockup-checkbox span { display: block; width: 24px; height: 24px; line-height: 18px; text-align: center; border: solid 2px #888; background-color: #f3f3f3; -webkit-border-radius: 4px; -moz-border-radius: 4px; border-radius: 4px;}label.auto-complete-mockup-checkbox span i { visibility: hidden; font-size: 16px !important; color: #888 !important;}label.auto-complete-mockup-checkbox.disabled { cursor: default;}label.auto-complete-mockup-checkbox.disabled span { border: solid 2px #ccc;}label.auto-complete-mockup-checkbox.on i { visibility: visible;}li.active label.auto-complete-mockup-checkbox-label { color: #fff;}</style>');
                var hasItem = false;
                ...
                if (list && list.length) {
                    ...
                    for (i = 0; i < list.length; i++) {
                        ...
                        hasItem = true;
                        stdItem = getStdItem ? getStdItem(item, "LIST") : item;
                        _$out_.push('<li data-id="', stdItem.id, '" data-index="', i, '" title="', $encodeHtml(getListItemText ? getListItemText(item, stdItem) : stdItem.name), '" style="text-overflow: ellipsis; overflow: hidden;"><a href="javascript:void(0);" onclick="return false;" data-rfl-mousedown="select ', i, '" data-rfl-mouseover="highlight ', i, '">');
                        if (checkbox) {
                            _$out_.push('<label class="auto-complete-mockup-checkbox-label"><label class="auto-complete-mockup-checkbox ', selectedIdHash[stdItem.id] ? "on" : "", '"><span><i class="icon-ok"></i></span></label><span>');
                        }
                        if (matchedInput) {
                            _$out_.push("", $encodeHtml((getListItemText ? getListItemText(item, stdItem) : stdItem.name).replace(new RegExp("(" + matchedInput.replace(/\\/, "\\\\").replace(/\./, "\\.") + ")", "ig"), "{{__AC_HL_START__}}$1{{__AC_HL_END__}}")).replace(/\{\{__AC_HL_START__\}\}/g, "<strong>").replace(/\{\{__AC_HL_END__\}\}/g, "</strong>"), "");
                        } else {
                            _$out_.push("", $encodeHtml(getListItemText ? getListItemText(item, stdItem) : stdItem.name), "");
                        }
                        if (checkbox) {
...
define("./rich-item-list.tpl.html", [ "require", "exports", "module" ], function(require, exports, module) {
    function $encodeHtml(str) {
        return (str + "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/`/g, "&#96;").replace(/'/g, "&#39;").replace(/"/g, "&quot;");
    }
    exports.render = function($data, $opt) {
        ...
        (function() {
            with ($data) {
                _$out_.push('<style type="text/css">.clearfix { *zoom: 1;}.clearfix:before,.clearfix:after { display: table; content: ""; line-height: 0;}.clearfix:after { clear: both;}.auto-complete-rich-box-list { cursor: text;}.auto-complete-rich-item { border: solid 1px #ccc; line-height: 30px; padding: 2px 6px; margin: 2px 0 0 2px; background-color: #f3f3f3; display: inline-block; cursor: default; -webkit-border-radius: 4px; -moz-border-radius: 4px; border-radius: 4px;}.auto-complete-rich-item.active { border: solid 1px #888;}.auto-complete-rich-item .icon-remove { cursor: pointer; color: #888;}</style><div class="auto-complete-rich-box-list">');
                var getStdItem = $opt.getStdItem;
                var getRichItemText = $opt.getRichItemText;
                for (var i = 0, l = list.length; i < l; i++) {
                    var item = list[i];
                    var stdItem = getStdItem ? getStdItem(item, "RICH_BOX") : item;
                    _$out_.push('<span data-id="', stdItem.id, '" class="auto-complete-rich-item">', $encodeHtml(getRichItemText ? getRichItemText(item, stdItem) : stdItem.name), ' <i class="icon-remove"></i></span>');
                }
                _$out_.push("</div>");
            }
        })();
...
define(['require', 'exports', 'module', 'jquery', './list.tpl.html', './rich-item-list.tpl.html'], function(require) {
	var $ = require('jquery') || jQuery;
	var listTpl = require('./list.tpl.html');
	var richItemListTpl = require('./rich-item-list.tpl.html');
	
	var AutoComplete = function(box, opt) {
		var self = this;
		opt = opt || {};
		this._opt = opt;
		this._box = $(box);
		this._richBox = null;
		this._list = null;
		this._maxSelection = opt.maxSelection > 0 ? opt.maxSelection : 9999;
	...
	$.extend(AutoComplete.prototype, {
		...
		_onKeydown: function(evt) {
			...
			if(keyCode === 40 && this._currentListData) {//down
				index = (this._currentListIndex + 1) % this._currentListData.length;
				...
				removeId = $('.active', this._richBox).data('id');
				if(removeId) {
					this.removeSelectedItem(removeId);
					if(this._checkbox) {
		...
		_bindEvent: function() {
			...
			this._list.delegate('li[data-index] a', 'mousedown', function(evt) {
				setTimeout(function() {//make sure the blur event of input box occurs first
					self._selectItem(parseInt($(evt.target).closest('li[data-index]').data('index')));
				}, 0);
			...
			if(lastRichItem) {
				...
				if(this._box.css('box-sizing') == 'border-box') {
					this._box.css({
						...
						height: top > 16 ? (this._box.data('initHeight') + top + 'px') : (this._box.data('initHeight') + 'px')
					});
				} else {
					this._box.css({
						paddingLeft: left + 'px',
