define("./available-list.tpl.html", [ "require", "exports", "module" ], function(require, exports, module) {
        ...
        (function() {
            with ($data) {
                ...
                for (var i = 0; i < list.length; i++) {
                    var item = list[i];
                    var stdItem = $opt.getStdItem ? $opt.getStdItem(item) : item;
                    _$out_.push('<li><span class="parent"><a class="text-gray" href="javascript:void(0);" onclick="return false;" data-rfl-click="add ', stdItem.id, '">', stdItem.name, '<i class="icon-share-alt"></i></a></span>');
                    if (stdItem.children && stdItem.children.length) {
                        _$out_.push("<ul>");
                        for (var j = 0; j < stdItem.children.length; j++) {
                            var childItem = stdItem.children[j];
                            var stdChildItem = $opt.getStdChildItem ? $opt.getStdChildItem(childItem) : childItem;
                            _$out_.push('<li><a class="text-gray" href="javascript:void(0);" onclick="return false;" data-rfl-click="addChild ', stdItem.id, " ", stdChildItem.id, '">', stdChildItem.name, '<i class="icon-share-alt"></i></a></li>');
                        }
                        _$out_.push("</ul>");
                    }
                    _$out_.push("</li>");
...
define("./selected-list.tpl.html", [ "require", "exports", "module" ], function(require, exports, module) {
    function $encodeHtml(str) {
        return (str + "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/`/g, "&#96;").replace(/'/g, "&#39;").replace(/"/g, "&quot;");
    }
    exports.render = function($data, $opt) {
        ...
        (function() {
            with ($data) {
                ...
                for (var i = 0; i < list.length; i++) {
                    var item = list[i];
                    var stdItem = $opt.getStdItem ? $opt.getStdItem(item) : item;
                    _$out_.push('<li><span class="parent"><a class="text-gray" href="javascript:void(0);" onclick="return false;" data-rfl-click="remove ', stdItem.id, '"><i class="icon-remove"></i>', stdItem.name, "</a></span>");
                    if (stdItem.children && stdItem.children.length) {
                        _$out_.push("<ul>");
                        for (var j = 0; j < stdItem.children.length; j++) {
                            var childItem = stdItem.children[j];
                            var stdChildItem = $opt.getStdChildItem ? $opt.getStdChildItem(childItem) : childItem;
                            _$out_.push('<li><a class="text-gray" href="javascript:void(0);" onclick="return false;" data-rfl-click="removeChild ', stdItem.id, " ", stdChildItem.id, '"><i class="icon-remove"></i>', stdChildItem.name, "</a></li>");
                        }
                        _$out_.push("</ul>");
                    }
                    _$out_.push("</li>");
...
define(['require', 'exports', 'module', 'jquery', 'rfl', './available-list.tpl.html', './selected-list.tpl.html'], function(require) {
	var $ = require('jquery');
	var rfl = require('rfl');
