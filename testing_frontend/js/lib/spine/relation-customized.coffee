...
AjaxAction =
  ajaxCreate: (record, options = {}) ->
    record.ajaxCreate $.extend(options, {
      url: options.url || @model.url(belongsTo: [Ajax.getLowerCaseModelName(@record.constructor.className) + 's', @record.id])
    })

  ajaxUpdate: (record, options = {}) ->
    params = options.ajax || {}
    if options.url
      url = options.url
    else
      url = @model.url({belongsTo: Ajax.getLowerCaseModelName(@record.constructor.className) + 's'}, record.id)
    record.ajaxUpdate $.extend(options, {
      url: url
    })

  ajaxDestroy: (record, options = {}) ->
    params = options.ajax || {}
    if options.url
      url = options.url
    else
      url = @model.url({belongsTo: Ajax.getLowerCaseModelName(@record.constructor.className) + 's'}, record.id)
    record.ajaxDestroy $.extend(options, {
      url: url
    })
  ...
  ajaxFetch: (options = {}) ->
    ...
    if options.url
      ...
      url = @model.url({belongsTo: Ajax.getLowerCaseModelName(@record.constructor.className) + 's'}, params.id)
    else
      url = @model.url(belongsTo: [Ajax.getLowerCaseModelName(@record.constructor.className) + 's', @record.id])
    @model.ajaxFetch $.extend(options, {
      url: url
    })
